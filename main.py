...
def add_dropdown_row():
    with dropdown_container:
        with ui.row().classes('items-center gap-4') as row_elem:
            ...
            row = {'dropdown': dropdown, 'input': input_box, 'checkbox': checkbox}
...
length_slider = ui.slider(min=8, max=22, value=8, on_change=lambda e: length_label.set_text(f"Length: {e.value}")).classes('w-64')
...
def update_output():
    password_table.rows = [
        {'password': p['password'], 'strength': strength_labels[p['lvl']]}
...
def handle_submit():
    ...
    for i in range(amount):
        ...
        level = check_password_entropy(new_pwd)
        print("Strength level:", level)

        # print(data,errMsg,dicts)
        print(password,new_pwd)
        password_generated.append({'password': new_pwd, 'lvl': level})
    update_output()
